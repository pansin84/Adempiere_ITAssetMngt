...
public class ProcessCopyEmployeedivision extends SvrProcess {

	public ProcessCopyEmployeedivision() {
		...
		try {
			...
			connection = DriverManager.getConnection(
			 ...
			 rs = st.executeQuery("SELECT j.name "
			 		...
			 		+ " from NV_EmployeeDivison j"
			 		+ " WHERE j.AD_Client_ID = 1000000");
			 
			 
			 while(rs.next()) {
				 int catID;
				 int cat[]=PO.getAllIDs("DM_EmployeeDivision", "Name='"+rs.getString("name").trim()+"' AND AD_Client_ID=1000000", get_TrxName());
