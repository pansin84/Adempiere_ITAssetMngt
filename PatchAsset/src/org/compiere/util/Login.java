...
public class Login
{
	 ...
	 *  The error (NoDatabase, UserPwdError, DBLogin) is saved in the log
	 */
	public KeyNamePair[] getRoles (String app_user, String app_pwd,boolean IsLoginUserID,boolean IsLoginEmail,boolean IsLoginEmpCode)
	{
		return getRoles (app_user, app_pwd, false,IsLoginUserID,IsLoginEmail,IsLoginEmpCode);
	}   //  login
	 ...
	 *  The error (NoDatabase, UserPwdError, DBLogin) is saved in the log
	 */
	private KeyNamePair[] getRoles (String app_user, String app_pwd, boolean force,boolean IsLoginUserID,boolean IsLoginEmail,boolean IsLoginEmpCode)
	{
		...
		else if (IsLoginEmpCode)
			userNameCol = "AD_User.EmployeeCode";
		else
			userNameCol = "COALESCE(AD_User.LDAPUser,AD_User.UserID)";
		...
		final String where = userNameCol+" = ? AND" +
        ...
        final ValueNamePair[]  passwordHash = DB.getValueNamePairs("SELECT  AD_User.AD_User_ID , AD_User.Password FROM AD_User  WHERE " + where , false  ,parameters);
		...
		StringBuffer sql = new StringBuffer("SELECT AD_User.AD_User_ID, r.AD_Role_ID,r.Name,")
			.append(" AD_User.ConnectionProfile ,AD_User.Name ")
			.append("FROM AD_User ")
			.append(" INNER JOIN AD_User_Roles ur ON (AD_User.AD_User_ID=ur.AD_User_ID AND ur.IsActive='Y')")
			.append(" INNER JOIN AD_Role r ON (ur.AD_Role_ID=r.AD_Role_ID AND r.IsActive='Y') ");
			if ( AD_User_ID > 0 )
				sql.append( "WHERE AD_User.AD_User_ID = ?");          // #1
			else 
				sql.append("WHERE "+userNameCol+"=?");		//	#1
			sql.append(" AND AD_User.IsActive='Y'")
			.append(" AND EXISTS (SELECT * FROM AD_Client c WHERE AD_User.AD_Client_ID=c.AD_Client_ID AND c.IsActive='Y')");
		if (app_pwd != null)
		...
		try
		{
			...
			Env.setContext(m_ctx, "#AD_User_Name", rs.getString(5));
