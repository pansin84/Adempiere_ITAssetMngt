	 ...
	 */
	 ...
	 protected boolean beforeSave (boolean newRecord){
		 int cnt = DB.getSQLValue(get_TrxName(),
					"SELECT COUNT(*) FROM M_Locator WHERE Value IS NOT NULL AND M_Warehouse_ID=? AND ISActive='Y' AND AD_Client_ID=? AND M_Locator_ID!=? "
					+ " ",
					getM_Warehouse_ID(), getAD_Client_ID(), get_ID());
			if (cnt > 0) {
				log.saveError("SaveError", "You can Create only one Locator Per Warehouse");
				return false;
			}
		 return true;
	}
