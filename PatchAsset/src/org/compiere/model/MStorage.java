...
public class MStorage extends X_M_Storage
{
	 ...
	 */
	public static MStorage get (Properties ctx, int M_Locator_ID, 
		int M_Product_ID, int M_AttributeSetInstance_ID,int M_Product_Category_ID,int Manufacturer_ID,int DM_AssetModelDetails_ID, String trxName)
	{
		MStorage retValue = null;
		String sql = "SELECT * FROM M_Storage "
			+ "WHERE M_Locator_ID=? AND M_Product_ID=? AND M_Product_Category_ID=? AND AssetManufacturer_ID = ? ";
//		if (M_AttributeSetInstance_ID == 0)
//			sql += "(M_AttributeSetInstance_ID=? OR M_AttributeSetInstance_ID IS NULL)";
//		else
//			sql += "M_AttributeSetInstance_ID=?";
		...
		try
		{
			...
			pstmt.setInt (4, Manufacturer_ID);
//			pstmt.setInt (5, M_AttributeSetInstance_ID);
		...
		boolean FiFo, boolean positiveOnly, int M_Locator_ID, String trxName)
	{
		...
		if (allAttributeInstances)
		{
			sql = "SELECT s.M_Product_ID,s.M_Locator_ID,s.M_AttributeSetInstance_ID,"
				+ "s.AD_Client_ID,s.AD_Org_ID,s.IsActive,s.Created,s.CreatedBy,s.Updated,s.UpdatedBy,"
				+ "s.QtyOnHand,s.QtyReserved,s.QtyOrdered,s.DateLastInventory "
				+ ",s.ModelNo,s.AssetManufacturer_ID ,s.DM_AssetModelDetails_ID,s.M_Product_Category_ID,s.M_Storage_ID "
				+ "FROM M_Storage s"
				+ " INNER JOIN M_Locator l ON (l.M_Locator_ID=s.M_Locator_ID)"
		...
		int M_Product_ID, int M_AttributeSetInstance_ID,int M_Product_Category_ID,int Manufacturer_ID,int DM_AssetModelDetails_ID ,String trxName)
	{
		...
		if (M_Product_ID == 0)
			throw new IllegalArgumentException("M_Product_ID=0");
		MStorage retValue = get(ctx, M_Locator_ID, M_Product_ID, M_AttributeSetInstance_ID,M_Product_Category_ID, Manufacturer_ID,DM_AssetModelDetails_ID, trxName);
		if (retValue != null)
			return retValue;
		...
		if (locator.get_ID() != M_Locator_ID)
			throw new IllegalArgumentException("Not found M_Locator_ID=" + M_Locator_ID);
		//
		retValue = new MStorage (locator, M_Product_ID, M_AttributeSetInstance_ID,M_Product_Category_ID, Manufacturer_ID,DM_AssetModelDetails_ID);
		retValue.save(trxName);
		s_log.fine("New " + retValue);
		return retValue;
	}	//	getCreate
	...
	public static boolean add (Properties ctx, int M_Warehouse_ID, int M_Locator_ID, 
		...
		boolean isldaio,boolean isprinter,boolean ispriaccessory,boolean isaccessory,boolean isnetcomp,
		boolean isinternet,int old_StorageDetails_ID,
		int DM_AssetModelDetails_ID,String DeviceSerialNo,int Manufacturer_ID,int M_Product_Category_ID,
		...
		BigDecimal diffQtyReserved, BigDecimal diffQtyOrdered, String trxName)
	{
		...
		if (storage == null)
			storage = getCreate (ctx, M_Locator_ID, 
				M_Product_ID, M_AttributeSetInstance_ID,M_Product_Category_ID,Manufacturer_ID,DM_AssetModelDetails_ID, trxName);
		//	Verify
		...
		if (prd.getM_AttributeSet_ID() == 0) {
			...
			reservationAttributeSetInstance_ID = M_AttributeSetInstance_ID;
		}
		//		
		...
		if (M_AttributeSetInstance_ID != reservationAttributeSetInstance_ID)
		{
			storage0 = get(ctx, M_Locator_ID, 
				M_Product_ID, reservationAttributeSetInstance_ID,M_Product_Category_ID,Manufacturer_ID,DM_AssetModelDetails_ID, trxName);
		...
		if (changed)
		{
			...
			if(DeviceSerialNo!=null){
				int storageid=0;
				if (storage0 != null)
					storageid=storage0.get_ID();
				else
					storageid=storage.get_ID();
				
				MStorage st = new MStorage(ctx, storageid, trxName);
				MStorageDetails sdet = new MStorageDetails(ctx, old_StorageDetails_ID, trxName);

				String WhereClause="AssetManufacturer_ID="+Manufacturer_ID+" AND M_Product_ID="+M_Product_ID+
						" AND M_Product_Category_ID="+M_Product_Category_ID+
						" AND M_Storage_ID="+storageid+
						" AND DeviceSerialNo='"+DeviceSerialNo+"'"+
						" AND AD_Client_ID="+Env.getAD_Client_ID(ctx);
				int[] storagedetailsID =  MStorageDetails.getAllIDs(MStorageDetails.Table_Name, WhereClause, trxName);
				if(storagedetailsID.length<=0){
					MAssetModelDetails det = new MAssetModelDetails(ctx, DM_AssetModelDetails_ID, trxName);
					MStorageDetails details= new MStorageDetails(ctx, 0, trxName);
					details.setM_Locator_ID(M_Locator_ID);
					details.setM_Product_Category_ID(M_Product_Category_ID);
					details.setM_Product_ID(M_Product_ID);
					details.setM_Storage_ID(storageid);
					details.setAssetManufacturer_ID(Manufacturer_ID);
					details.setDeviceSerialNo(DeviceSerialNo);
					details.setModelNo(st.getModelNo());
					details.setAD_Org_ID(st.getAD_Org_ID());
					details.setDM_AssetModelDetails_ID(det.get_ID());
					if(isldaio){
						details.setIsLaptopDektop(true);
						details.setHDD(sdet.getHDD());
						details.setRAM(sdet.getRAM());
						details.setGraphicsCard(sdet.getGraphicsCard());
						details.setIsCDROMAvailable(sdet.isCDROMAvailable());
						details.setProcessor(sdet.getProcessor());
						details.setScreenSize(sdet.getScreenSize());
					}
					if(isprinter){
						details.setIsPrinter(true);
						details.setIsCanCopy(sdet.isCanCopy());
						details.setIsCanPrint(sdet.isCanPrint());
						details.setIsCanScan(sdet.isCanScan());
						details.setIsWifiSupport(sdet.isWifiSupport());
						details.setIsColorPrinter(sdet.isColorPrinter());
						details.setInputTrayCapacity(sdet.getInputTrayCapacity());
					}
					if(ispriaccessory){
						details.setIsPrinterAccessory(true);
					}
					if(isaccessory){
						details.setIsLDAccessory(true);
					}
					if(isnetcomp){
						details.setIsNetComp(true);
					}
					if(isinternet){
						details.setIsInternet(true);
						details.setMobilePhone(sdet.getMobilePhone());
					}
					if (diffQtyOnHand != null && diffQtyOnHand.signum() != 0)
					{
						details.setQtyAvailable (details.getQtyAvailable().add (diffQtyOnHand));
						
					}
					if(details.save()){}
				}else{
					MStorageDetails details= new MStorageDetails(ctx, storagedetailsID[0], trxName);
					if (diffQtyOnHand != null && diffQtyOnHand.signum() != 0)
					{
						details.setQtyAvailable (details.getQtyAvailable().add (diffQtyOnHand));
	...
	public static boolean add (Properties ctx, int M_Warehouse_ID, int M_Locator_ID, 
		int M_Product_ID, int M_AttributeSetInstance_ID, int reservationAttributeSetInstance_ID,
		boolean isldaio,boolean isprinter,boolean ispriaccessory,boolean isaccessory,boolean isnetcomp,
		boolean isuniquesrno,boolean isinternet,String mobile,BigDecimal screensize,
		int DM_AssetModelDetails_ID,String DeviceSerialNo,int Manufacturer_ID,int RAM,int HDD,int GCard,
		boolean cdrom,String Processor,int M_Product_Category_ID,int employeeid,
		BigDecimal diffQtyOnHand, 
		BigDecimal diffQtyReserved, BigDecimal diffQtyOrdered, String trxName,String movementType
		,int m_inout_id, MInOutLine inoutline)
	{
		MStorage storage = null;
		StringBuffer diffText = new StringBuffer("(");
			...
			if(DeviceSerialNo!=null){
				...
				if(storagedetailsID.length<=0){
					...
					if(isldaio){
						...
						details.setHDD(HDD);
						details.setRAM(RAM);
						details.setGraphicsCard(GCard);
						details.setIsCDROMAvailable(cdrom);
						details.setProcessor(Processor);
						details.setScreenSize(screensize);
						if(inoutline.getOsInstalled()!=null){
							details.setOsInstalled(inoutline.getOsInstalled());
						}
						if(inoutline.getOsSerialNo()!=null){
							details.setOsSerialNo(inoutline.getOsSerialNo());
						}
					}
					if(isprinter){
						details.setIsPrinter(true);
						details.setIsCanCopy(det.isCanCopy());
						details.setIsCanPrint(det.isCanPrint());
						details.setIsCanScan(det.isCanScan());
						details.setIsWifiSupport(det.isWifiSupport());
						details.setIsColorPrinter(det.isColorPrinter());
						details.setInputTrayCapacity(det.getInputTrayCapacity());
					}
					if(ispriaccessory){
						details.setIsPrinterAccessory(true);
					}
					...
					if(details.save()){
						if(MInOut.MOVEMENTTYPE_CustomerShipment.equals(movementType) || MInOut.MOVEMENTTYPE_CustomerReturns.equals(movementType) ||  MInOut.MOVEMENTTYPE_VendorReturns.equals(movementType)){
							saveItemTrx(ctx,trxName,employeeid,movementType,details,diffQtyOnHand,m_inout_id,inoutline);
						}
					}
				}else{
					MStorageDetails details= new MStorageDetails(ctx, storagedetailsID[0], trxName);
	...
	public static boolean saveItemTrx(Properties ctx,String trxName,int imployeeid,String MovementType
			,MStorageDetails sdetails,BigDecimal diffQtyOnHand,int m_inout_id, MInOutLine inoutline){
		String WhereClause="DM_DMSEmployee_ID="+imployeeid+" AND DM_StorageDetails_ID="+sdetails.get_ID()+
				" AND M_Storage_ID="+sdetails.getM_Storage_ID()+
				" AND AD_Client_ID="+Env.getAD_Client_ID(ctx);
		int[] storagedetailsID =  MAssetTransaction.getAllIDs(MAssetTransaction.Table_Name, WhereClause, trxName);
		MAssetTransaction trxn=null;
		if(storagedetailsID.length<=0){
			trxn=new MAssetTransaction(ctx, 0, trxName);
			trxn.setM_Storage_ID(sdetails.getM_Storage_ID());
			trxn.setDM_StorageDetails_ID(sdetails.get_ID());
			trxn.setDM_DMSEmployee_ID(imployeeid);
			trxn.setAD_Org_ID(sdetails.getAD_Org_ID());
			trxn.save();
		}else{
			trxn=new MAssetTransaction(ctx, storagedetailsID[0], trxName);		
		}
		if(MInOut.MOVEMENTTYPE_CustomerShipment.equals(MovementType)){
			if(sdetails.isLaptopDektop() || sdetails.isPrinter()){
				if(trxn.isReturned()){
					StringBuffer sb = new StringBuffer();
					sb.append("Previous Issue date : "+trxn.getDateIssued()).append("\n");
					sb.append("Previous return date : "+trxn.getDateReturned()).append("\n");
					if(trxn.getRemarks()!=null){
						sb.append(trxn.getRemarks()).append("\n\n\n");
					}
					trxn.setRemarks(sb.toString());
					trxn.setQtyIssued(Env.ZERO);
					trxn.setQtyReturned(Env.ZERO);
					trxn.setIsReturned(false);
					trxn.setReturnedBy_ID(-1);
					trxn.setDateReturned(null);
					trxn.save();
				}
			}
			trxn.setDateIssued(new Timestamp(System.currentTimeMillis()));
			trxn.setIssuedBy_ID(Env.getAD_User_ID(ctx));
			if (diffQtyOnHand != null && diffQtyOnHand.signum() != 0)
			{
				trxn.setQtyIssued(trxn.getQtyIssued().subtract (diffQtyOnHand));
				
			}
			if(trxn.save()){
				String WhereCl="DM_DMSEmployee_ID="+imployeeid+" AND AssetManufacturer_ID="+sdetails.getAssetManufacturer_ID()+
						//" AND DM_AssetModelDetails_ID="+sdetails.getDM_AssetModelDetails_ID()+
						" AND M_Product_Category_ID="+sdetails.getM_Product_Category_ID()+
						" AND DeviceSerialNo='"+sdetails.getDeviceSerialNo()+"'"+
						" AND AD_Client_ID="+Env.getAD_Client_ID(ctx);
				int[] empissueID =  MIssuedToEmployee.getAllIDs(MIssuedToEmployee.Table_Name, WhereCl, trxName);
				if(empissueID.length<=0){
					MIssuedToEmployee issue = new MIssuedToEmployee(ctx, 0, trxName);
					issue.setDM_DMSEmployee_ID(imployeeid);
					issue.setDeviceSerialNo(sdetails.getDeviceSerialNo());
					issue.setDM_AssetModelDetails_ID(sdetails.getDM_AssetModelDetails_ID());
					issue.setM_Product_Category_ID(sdetails.getM_Product_Category_ID());
					issue.setM_Product_ID(sdetails.getM_Product_ID());
					issue.setAssetManufacturer_ID(sdetails.getAssetManufacturer_ID());
					issue.setM_InOut_ID(m_inout_id);
					issue.setM_InOutLine_ID(inoutline.get_ID());
					if (diffQtyOnHand != null && diffQtyOnHand.signum() != 0)
					{
						issue.setQtyIssued(issue.getQtyIssued().subtract (diffQtyOnHand));
						
					}
					if(issue.save()){
						if(issue.getQtyIssued().compareTo(Env.ZERO)==0){
							issue.delete(true);
						}
					}
				}else{
					MIssuedToEmployee issue = new MIssuedToEmployee(ctx, empissueID[0], trxName);
					if (diffQtyOnHand != null && diffQtyOnHand.signum() != 0)
					{
						issue.setQtyIssued(issue.getQtyIssued().subtract (diffQtyOnHand));
		...
		if(MInOut.MOVEMENTTYPE_CustomerReturns.equals(MovementType)){
			trxn.setDateReturned(new Timestamp(System.currentTimeMillis()));
			trxn.setReturnedBy_ID(Env.getAD_User_ID(ctx));
			trxn.setIsReturned(true);
			if (diffQtyOnHand != null && diffQtyOnHand.signum() != 0)
			{
				trxn.setQtyReturned(trxn.getQtyReturned().add (diffQtyOnHand));				
			}
			if(trxn.save()){
				String WhereCl="DM_DMSEmployee_ID="+imployeeid+" AND AssetManufacturer_ID="+sdetails.getAssetManufacturer_ID()+
						//" AND DM_AssetModelDetails_ID="+sdetails.getDM_AssetModelDetails_ID()+
	...
	/**
	 * ITem Type Printer or accessory
	 * 	Update Storage Info add.
	 * 	Called from MProjectIssue
	 *	@param ctx context
	 *	@param M_Warehouse_ID warehouse
	...
	public static boolean add (Properties ctx, int M_Warehouse_ID, int M_Locator_ID, 
		int M_Product_ID, int M_AttributeSetInstance_ID, int reservationAttributeSetInstance_ID,
		int DM_AssetModelDetails_ID,String DeviceSerialNo,int Manufacturer_ID,boolean printer,
		BigDecimal diffQtyOnHand, 
		BigDecimal diffQtyReserved, BigDecimal diffQtyOrdered, String trxName)
	{
		return true;
	 ...
	 *	@param trxName transaction
	 */
	public MStorage (Properties ctx, int M_Storage_ID,int ignored, String trxName)
	{
		super(ctx, 0, trxName);
		if (ignored != 0)
	...
	private MStorage (MLocator locator, int M_Product_ID, int M_AttributeSetInstance_ID,int M_Product_category_ID,int Manufacturer_ID,int DM_AssetModelDetails_ID)
	{
		this (locator.getCtx(), 0, locator.get_TrxName());
		setClientOrg(locator);
		setM_Locator_ID (locator.getM_Locator_ID());
		setM_Product_ID (M_Product_ID);
		setM_AttributeSetInstance_ID (M_AttributeSetInstance_ID);
		setM_Product_Category_ID(M_Product_category_ID);
		setAssetManufacturer_ID(Manufacturer_ID);
		MAssetModelDetails det = new MAssetModelDetails(getCtx(), DM_AssetModelDetails_ID, get_TrxName());
		if(det!=null){
			setModelNo(det.getModelNo());
			setDM_AssetModelDetails_ID(det.get_ID());
		}
	}	//	MStorage
