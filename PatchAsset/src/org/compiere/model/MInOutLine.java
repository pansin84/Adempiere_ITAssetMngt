...
//		}
		...
		if(isSerialNoReq()){
			if(getDeviceSerialNo()!=null){
				int cnt = DB.getSQLValue(get_TrxName(),
						"SELECT COUNT(*) FROM M_InOutLine WHERE DeviceSerialNo IS NOT NULL AND lower(DeviceSerialNo)=?   AND ISActive='Y' AND AD_Client_ID=? AND M_InOutLine_ID!=? "
						+ " ",
						getDeviceSerialNo().toLowerCase().trim(), getAD_Client_ID(), get_ID());
				if (cnt > 0) {
					Object[] args = new Object[] {"Not Unique ","Serial No ", getDeviceSerialNo()};
					log.saveError("SaveError", Msg.getMsg(getCtx(), "SAVE_ERROR_NOT_UNIQUE_MSG",args) +" - "+ "Device Serial No"+"("+getDeviceSerialNo()+")");
					return false;
				}
			}
		}
		if(getDeviceSerialNo()!=null)
			setDeviceSerialNo(getDeviceSerialNo().trim());
		
		if(getQtyEntered().intValue()==0){
			log.saveError("SaveError","Entered Quantity must not be Zero (0)");
			return false;
		}
		
		 if(getM_InOut().getMovementType().equals(MInOut.MOVEMENTTYPE_CustomerReturns)){
			 MStorageDetails det = new MStorageDetails(getCtx(), getDM_StorageDetails_ID(), get_TrxName());
			 setDeviceSerialNo(det.getDeviceSerialNo());
		 }
		 if(getM_InOut().getMovementType().equals(MInOut.MOVEMENTTYPE_CustomerShipment)){
			 if(getDM_StorageDetails_ID()>0){
				 MStorageDetails det = new MStorageDetails(getCtx(), getDM_StorageDetails_ID(), get_TrxName());
				 setDeviceSerialNo(det.getDeviceSerialNo());
				 if(getQtyEntered().compareTo(det.getQtyAvailable())>0){
					 log.saveError("SaveError","Entered Quantity must be same or less than Available Quantity");
						return false;
				 }
			 }
		 }
