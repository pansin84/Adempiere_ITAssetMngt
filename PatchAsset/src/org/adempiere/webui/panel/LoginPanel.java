...
public class LoginPanel extends Window implements EventListener
{
    ...
    private Label lblempcodeoremail;
    private Radiogroup rempcodeoremail;
    private Radio empcode;
    private Radio email;
    private Radio username;
    ...
    private void init()
    {
        ...
        tr = new Tr();
        tr.setId("rowLoginType");
        table.appendChild(tr);
    	td = new Td();
    	tr.appendChild(td);
    	td.setSclass(ITheme.LOGIN_LABEL_CLASS);
    	td.appendChild(lblempcodeoremail);
    	td = new Td();
    	td.setSclass(ITheme.LOGIN_FIELD_CLASS);
    	tr.appendChild(td);
    	td.appendChild(rempcodeoremail);
    ...
    {
    	lblempcodeoremail=new Label();
    	lblempcodeoremail.setId("lblempcodeoremail");
    	lblempcodeoremail.setValue("Login With");
    	
    	rempcodeoremail=new Radiogroup();
    	rempcodeoremail.setId("rempcodeoremail");
    	rempcodeoremail.setOrient("horizontal");
    	rempcodeoremail.setName("rempcodeoremail");
    	
    	username = new Radio();
    	rempcodeoremail.appendChild(username);
    	username.setId("username");
    	username.setName("username");
    	username.setLabel("User Name");
    	username.setChecked(true);
    	username.addEventListener(Events.ON_CHECK,this);
	    
    	empcode = new Radio();
    	rempcodeoremail.appendChild(empcode);
	    empcode.setId("empcode");
	    empcode.setName("empcode");
	    empcode.setLabel("Employee Code");
	    empcode.setChecked(false);
	    empcode.addEventListener(Events.ON_CHECK,this);
	    
	    email = new Radio();
	    rempcodeoremail.appendChild(email);
	    email.setId("email");
	    email.setName("email");
	    email.setLabel("Email");
	    email.setChecked(false);
	    email.addEventListener(Events.ON_CHECK,this);
...
//        txtPassword.setMaxlength(40);
    ...
    public void onEvent(Event event)
    {
        ...
        if (event.getName().equals(Events.ON_CHECK))
        {
        	if(event.getTarget() instanceof Radio)
            {
        		...
        		else if ("email".equals(btn.getId()))
                {
        			LoginuserName=false;
        			LoginEmail=true;
        			LoginEmpCode=false;
        			lblUserId.setValue("EMail ID");
                }
        		else{
        			LoginuserName=false;
        			LoginEmail=false;
        			LoginEmpCode=true;
        			lblUserId.setValue("Employee Code");
        		}
            }
        }
    ...
    **/
    public void validateLogin()
    {
        ...
        KeyNamePair rolesKNPairs[] = login.getRoles(userId, userPassword,LoginuserName,LoginEmail,LoginEmpCode);
        if(rolesKNPairs == null || rolesKNPairs.length == 0){
        	if(LoginuserName)
        		throw new WrongValueException("Login is invalid, UserID: " + userId + " and Password:" + userPassword);
        	else if(LoginEmail)
        		throw new WrongValueException("Login is invalid, EMail: " + userId + " and Password:" + userPassword);
        	else
        		throw new WrongValueException("Login is invalid, Employee Code: " + userId + " and Password:" + userPassword);
        }
        else
        {
        	...
        	Language language = findLanguage(langName);
            wndLogin.loginOk(userId, userPassword,LoginuserName,LoginEmail,LoginEmpCode);
