...
import org.compiere.model.MInventoryAttachment;
import org.compiere.model.MInventoryAttachmentEntry;
...
public class WInventoryAttachment extends Window implements EventListener
{
	/**
	 * 
	 */
	...
	/** Attachment				*/
	private MInventoryAttachment	m_attachment;
	 ...
	 *  @param DM_InventoryAttachment_ID attachment
	 *  @param AD_Table_ID table
	 *  @param Record_ID record key
	 *  @param trxName transaction
	 */
	
	public WInventoryAttachment(	int WindowNo, int DM_InventoryAttachment_ID,
						int AD_Table_ID, int Record_ID, String trxName, String Type)
	{
		super();
		
		log.config("ID=" + DM_InventoryAttachment_ID + ", Table=" + AD_Table_ID + ", Record=" + Record_ID+", type="+Type);
		...
		if (DM_InventoryAttachment_ID <= 0)
			m_attachment = new MInventoryAttachment (Env.getCtx(), AD_Table_ID, Record_ID, trxName, Type);
		else
			m_attachment = new MInventoryAttachment (Env.getCtx(), DM_InventoryAttachment_ID, trxName, Type);
	...
	public void dispose (Event e)
	{
		preview = null;	
		e.stopPropagation();
		this.setVisible(false);
		Events.echoEvent("onCloseLater", this, null);
	...
	public void displaySelected() {
		MInventoryAttachmentEntry entry = m_attachment.getEntry(displayIndex); 
...
//				pdfViewer.setContent(media);
	...
	private void saveAttachmentToFile()
	{
		...
		MInventoryAttachmentEntry entry = m_attachment.getEntry(index);
		if (entry != null && entry.getData() != null)
		{
			try
			{
