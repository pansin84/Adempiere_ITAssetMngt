...
public class RolePanel extends Window implements EventListener, Deferrable
{
	...
	private boolean m_LoginuserName;
	private boolean m_LoginEmail;
	private boolean m_LoginEmpCode;
	...
	public RolePanel(Properties ctx, LoginWindow loginWindow, String userName, String password,boolean LoginuserName,boolean LoginEmail,boolean LoginEmpCode)
    {
        this.wndLogin = loginWindow;
        m_ctx = ctx;
        m_userName = userName;
        m_password = password;
        this.m_LoginuserName=LoginuserName;
        this.m_LoginEmail=LoginEmail;
        this.m_LoginEmpCode=LoginEmpCode;
        login = new Login(ctx);
        rolesKNPairs = login.getRoles(userName, password,LoginuserName,LoginEmail,LoginEmpCode);
        if(rolesKNPairs == null)
            throw new ApplicationException("Login is invalid, UserName: " + userName + " and Password:" + password);
    ...
    private void updateClientList()
    {
        ...
        login.getRoles(m_userName, m_password,m_LoginuserName,m_LoginEmail,m_LoginEmpCode);
