...
//			pstmt.close();
	 ...
	 *	@param Record_ID
	 */
	public static void zoomItemIssue (int AD_Table_ID, int Record_ID)
	{
		String TableName = null;
		int AD_Window_ID = 0;
...
//			pstmt.close();
    ...
    * @param value
    */
	protected static Object 	Code;
   public static void actionReport(Lookup lookup, Object code)
   {
       if (lookup == null)
           return;
       ...
       if (zoomQuery == null || code != null)
       {
       	...
       	if (lookup instanceof MLookup && DisplayType.List == lookup.getDisplayType())
       	{
       		int AD_Reference_ID = ((MLookup)lookup).getAD_Reference_Value_ID();
       		column = "AD_Ref_List_ID";
       		code = DB.getSQLValue(null, "SELECT AD_Ref_List_ID FROM AD_Ref_List WHERE AD_Reference_ID=? AND Value=?", AD_Reference_ID, code);
       	}
           //strip off table name, fully qualify name doesn't work when zoom into detail tab
           if (column.indexOf(".") > 0)
           {
           	int p = column.indexOf(".");
           	tableName = column.substring(0, p);
           	column = column.substring(column.indexOf(".")+1);
           	zoomQuery.setZoomTableName(tableName);
           	zoomQuery.setZoomColumnName(column);
           }
           else
           {
           	zoomQuery.setZoomColumnName(column);
           	//remove _ID to get table name
           	tableName=column.substring(0, column.length() - 3);
           	zoomQuery.setZoomTableName(column.substring(0, column.length() - 3));
           }
           zoomQuery.setZoomValue(code);
           zoomQuery.addRestriction(column, MQuery.EQUAL, code);
           zoomQuery.setRecordCount(1);    //  guess
       }
       ...
       int windowId = lookup.getZoom(zoomQuery);
       String processID="AD_Process_ID";
      
       String sql="SELECT "+processID+" from AD_Tab tab" +
    		   " inner join ad_table tabl "+
    		   " on(tabl.ad_table_id=tab.ad_table_id and tabl.ad_window_id=tab.ad_window_id)"+
       			" where tabl.ad_table_id=(select ad_table_id from ad_table where tablename='"+tableName+"')";
       int process_id=DB.getSQLValue(null, sql);
       if(process_id>0){
    	   MProcess process =MProcess.get(Env.getCtx(), process_id);
    	   if(process.getJasperReport()!=null){
    		   ProcessModalDialog dialog = new ProcessModalDialog(null,lookup.getWindowNo(),
    	    		   process_id,DB.getSQLValue(null, "select ad_table_id from ad_table where tablename='"+tableName+"'"), (Integer) Code, true);
    			if (dialog.isValid()) {
    		   ...
    		   FDialog.error(lookup.getWindowNo(), "Jasper Report Not Found");
    		   return;
    	   }
       }else{
    	   FDialog.error(lookup.getWindowNo(), "Report Not Found");
    	   return;
       }
