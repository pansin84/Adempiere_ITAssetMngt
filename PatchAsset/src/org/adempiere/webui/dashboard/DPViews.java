...
public class DPViews extends DashboardPanel implements EventListener {
	...
	private Box createViewPanel()
	{
		...
		if (MRole.getDefault().isAllow_Info_BPartner())
		{
			ToolBarButton btnViewItem = new ToolBarButton("InfoEmployee");
			btnViewItem.setLabel(Util.cleanAmp(Msg.getMsg(Env.getCtx(), "InfoEmployee")));
			btnViewItem.setImage("/images/InfoEmployee16.png");
			btnViewItem.addEventListener(Events.ON_CLICK, this);
			vbox.appendChild(btnViewItem);
		}
    ...
    public void onEvent(Event event)
    {
        ...
        if(eventName.equals(Events.ON_CLICK))
        {
            if(comp instanceof ToolBarButton)
            {
        		...
        		else if (actionCommand.equals("InfoEmployee") && AEnv.canAccessInfo("EMPLOYEE"))
        		{
        			InfoPanel.showEmployee(WindowNo);
        		}
